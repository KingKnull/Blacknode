// @ts-check
// Cynhyrchwyd y ffeil hon yn awtomatig. PEIDIWCH Â MODIWL
// This file is automatically generated. DO NOT EDIT

/**
 * ContainerService runs `docker` and `kubectl` over SSH against a connected
 * host. We deliberately do NOT depend on a local docker/kubectl install —
 * every command runs on the remote box, which is the only place that has the
 * real socket/kubeconfig anyway.
 * @module
 */

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore: Unused imports
import { Call as $Call, CancellablePromise as $CancellablePromise, Create as $Create } from "@wailsio/runtime";

// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore: Unused imports
import * as $models from "./models.js";

/**
 * ContainerLogs returns the last `lines` of logs for one-shot display. For
 * streaming, use the existing LogsService with `docker logs -f <id>`.
 * @param {string} hostID
 * @param {string} containerID
 * @param {number} lines
 * @returns {$CancellablePromise<string>}
 */
export function ContainerLogs(hostID, containerID, lines) {
    return $Call.ByID(2614646323, hostID, containerID, lines);
}

/**
 * Containers runs `docker ps --format json` and parses the per-line JSON
 * docker emits in newer versions. Falls back to a parsed table from
 * `docker ps --format ...` if `--format json` isn't supported.
 * @param {string} hostID
 * @param {boolean} includeStopped
 * @returns {$CancellablePromise<$models.Container[]>}
 */
export function Containers(hostID, includeStopped) {
    return $Call.ByID(1663870777, hostID, includeStopped).then(/** @type {($result: any) => any} */(($result) => {
        return $$createType1($result);
    }));
}

/**
 * Namespaces returns the list of kubernetes namespaces visible on the host.
 * @param {string} hostID
 * @returns {$CancellablePromise<string[]>}
 */
export function Namespaces(hostID) {
    return $Call.ByID(1658735729, hostID).then(/** @type {($result: any) => any} */(($result) => {
        return $$createType2($result);
    }));
}

/**
 * PodLogs returns the last N lines of logs for a pod (optionally a specific
 * container within it). Streaming is via the existing LogsService.
 * @param {string} hostID
 * @param {string} $namespace
 * @param {string} pod
 * @param {string} container
 * @param {number} lines
 * @returns {$CancellablePromise<string>}
 */
export function PodLogs(hostID, $namespace, pod, container, lines) {
    return $Call.ByID(879136655, hostID, $namespace, pod, container, lines);
}

/**
 * Pods runs `kubectl get pods -o json` for a namespace and parses the
 * minimal subset we render. Empty namespace = all namespaces.
 * @param {string} hostID
 * @param {string} $namespace
 * @returns {$CancellablePromise<$models.Pod[]>}
 */
export function Pods(hostID, $namespace) {
    return $Call.ByID(3677389221, hostID, $namespace).then(/** @type {($result: any) => any} */(($result) => {
        return $$createType4($result);
    }));
}

// Private type creation functions
const $$createType0 = $models.Container.createFrom;
const $$createType1 = $Create.Array($$createType0);
const $$createType2 = $Create.Array($Create.Any);
const $$createType3 = $models.Pod.createFrom;
const $$createType4 = $Create.Array($$createType3);
