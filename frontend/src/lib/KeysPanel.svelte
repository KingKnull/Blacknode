  ...
  {#if creating}
    <div class="border-b hairline surface-1 p-4">
      <div class="flex items-center gap-2">
        <h3 class="type-caption font-medium text-[var(--color-text-1)]">
          Generate keypair
        </h3>
        <button
      ...
      <div class="mt-2 flex items-center gap-2">
        <input
          class="flex-1 rounded-md border hairline bg-[var(--color-surface-3)] px-3 py-1.5 type-caption outline-none"
          bind:value={newName}
          placeholder="key name"
        />
        <select
          class="rounded-md border hairline bg-[var(--color-surface-3)] px-3 py-1.5 type-caption outline-none"
          bind:value={newType}
        >
  ...
  {#if importing}
    <div class="space-y-2 border-b hairline surface-1 p-4">
      ...
      <div class="flex gap-2">
        ...
        <input
          type="password"
          class="w-48 rounded-md border hairline bg-[var(--color-surface-3)] px-3 py-1.5 type-caption outline-none"
          bind:value={importPass}
          placeholder="passphrase (optional)"
        />
      </div>
      <textarea
        class="h-32 w-full rounded-md border hairline bg-[var(--color-surface-3)] px-3 py-2 font-mono type-caption outline-none"
      ...
      {#if err}
        <p class="type-caption text-[var(--color-danger)]">{err}</p>
      {/if}
    </div>
  {/if}
      ...
      {#each app.keys as k (k.id)}
        ...
        >
          <div class="flex items-center gap-2">
            ...
            <span class="type-body font-medium text-[var(--color-text-1)]"
              >{k.name}</span
            >
            <span
          ...
          <pre
            class="mt-2 overflow-x-auto rounded bg-[var(--color-code-bg)] px-3 py-2 font-mono type-micro text-[var(--color-text-3)]">{k.publicKey.trim()}</pre>
        </div>
      {/each}
    </div>
