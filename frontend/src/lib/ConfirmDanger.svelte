...
>
  {#snippet children()}
    ...
    <div class="space-y-3 p-5">
      <p class="text-sm leading-relaxed text-[var(--color-text-2)]">{body}</p>

      {#if productionHosts.length > 0}
        <div class="border border-[var(--color-danger)]/25 bg-[var(--color-danger)]/8 p-3">
          <div class="text-xs font-semibold text-[var(--color-danger)] mb-2">Production hosts in scope</div>
          <div class="flex flex-wrap gap-1">
            {#each productionHosts as h (h)}
              <span class="border border-[var(--color-danger)]/40 bg-[var(--color-danger)]/12 px-1.5 py-0.5 font-mono text-xs text-[var(--color-danger)]">{h}</span>
            {/each}
          </div>
        </div>
      {/if}
      ...
      {#if requirePhrase}
        <label class="block">
          <span class="text-xs text-[var(--color-text-4)]">
    ...
    >
      <button class="border border-[var(--color-line)] px-3 py-1.5 text-sm text-[var(--color-text-3)] hover:border-[var(--color-line-strong)] hover:text-[var(--color-text-1)] transition-all"
        onclick={onCancel}>Cancel</button>
      <button
        class="flex items-center gap-1.5 border border-[var(--color-danger)]/50 bg-[var(--color-danger)]/12 px-3 py-1.5 text-sm font-semibold text-[var(--color-danger)] hover:bg-[var(--color-danger)]/20 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
        disabled={!canConfirm}
        onclick={onConfirm}
      >
        <ShieldAlert size="12" />
