...
<div class="flex h-full w-full flex-col">
  ...
  <div class="flex items-center gap-2 border-b hairline px-3 py-2">
    <span class="type-caption font-semibold text-[var(--color-text-3)]">Hosts</span>
    <button
  ...
  <div class="border-b hairline px-2 py-2">
    <div class="relative flex items-center border hairline surface-2 focus-within:border-[var(--color-accent)]/50 transition-colors" style="border-radius: var(--radius-sm);">
      <Search size="10" class="absolute left-2 text-[var(--color-text-4)]" />
      <input
        class="w-full bg-transparent py-1.5 pl-6 pr-2 font-mono type-caption outline-none placeholder:text-[var(--color-text-4)] text-[var(--color-text-2)]"
  ...
  <div class="flex-1 overflow-y-auto pb-2">
    {#each Object.entries(groups) as [name, list] (name)}
      <div class="px-3 pt-3 pb-1 flex items-center gap-2">
        ...
        <span class="type-micro text-[var(--color-text-4)] font-mono">{name}</span>
        <span class="h-px flex-1 bg-[var(--color-line)]"></span>
      </div>
      {#each list as h (h.id)}
        ...
        >
          ...
          >
            ...
            <div class="min-w-0 flex-1">
              ...
              <div class="clamp-1 font-mono text-[9px] text-[var(--color-text-4)]">
    ...
    {#if app.hosts.length === 0}
      <div class="px-4 py-10 text-center">
        <div class="mx-auto mb-3 flex h-12 w-12 items-center justify-center border hairline text-[var(--color-text-4)]" style="border-radius: var(--radius-sm); box-shadow: 0 0 20px rgba(0,255,136,0.04);">
          <Server size="20" class="glow-pulse" />
        </div>
        <p class="type-caption text-[var(--color-text-3)] font-medium">No hosts yet</p>
        <p class="mt-1 type-caption text-[var(--color-text-4)]">Add your first server to get started</p>
        <button
          class="mt-3 border hairline px-3 py-1.5 type-caption text-[var(--color-accent)]/70 hover:border-[var(--color-accent)]/40 hover:text-[var(--color-accent)] hover:bg-[var(--color-accent)]/5 transition-all"
          style="border-radius: var(--radius-sm);"
          onclick={() => (creating = true)}
        >+ Add host</button>
      </div>
...
{#if menuHostID}
  ...
  ></div>
  <div
    class="fade-up fixed z-50 min-w-[140px] overflow-hidden border hairline-strong surface-2 py-0.5 font-mono type-micro shadow-xl shadow-black/60"
    style="left: {menuPos.x}px; top: {menuPos.y}px; border-radius: var(--radius-sm);"
  >
    <button
