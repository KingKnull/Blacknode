...
<div class="flex h-full flex-col surface-1 border-l hairline">
  <div
    class="flex items-center gap-2 border-b hairline px-3 py-2 type-caption"
  >
    ...
    <div class="ml-2 flex items-center gap-0.5 rounded-md border hairline p-0.5">
      <button
        class="flex items-center gap-1 rounded px-1.5 py-0.5 type-micro {mode === 'translate' ? 'bg-[var(--color-surface-3)] text-[var(--color-text-1)]' : 'text-[var(--color-text-3)] hover:text-[var(--color-text-1)]'}"
      ...
      <button
        class="flex items-center gap-1 rounded px-1.5 py-0.5 type-micro {mode === 'explain' ? 'bg-[var(--color-surface-3)] text-[var(--color-text-1)]' : 'text-[var(--color-text-3)] hover:text-[var(--color-text-1)]'}"
  ...
  <div bind:this={scrollEl} class="flex-1 overflow-y-auto px-3 py-2 type-caption">
    {#if !app.settings.hasAnthropicKey}
      <div class="m-auto flex h-full flex-col items-center justify-center gap-2 text-center">
        ...
        <button
          class="flex items-center gap-1 rounded-md bg-[var(--color-accent)] px-3 py-1.5 type-caption font-medium text-[var(--color-surface-0)] hover:opacity-90"
        ...
        {#each messages as m, i (i)}
          {#if m.role === "user"}
            <div class="rounded-md border hairline surface-2 px-3 py-2">
              <div class="type-eyebrow text-[var(--color-text-4)]">
                you · {m.kind}
              </div>
          ...
          {:else}
            ...
            >
              <div
                class="flex items-center gap-1 type-eyebrow text-[var(--color-accent)]"
              >
              ...
              {#if m.kind === "translate"}
                <pre
                  class="mt-1 overflow-x-auto rounded bg-[var(--color-code-bg)] px-2 py-1.5 font-mono type-caption text-[var(--color-text-1)]">{m.text}</pre>
                {#if m.text}
                  <div class="mt-2 flex gap-1">
                    <button
                      class="rounded-md bg-[var(--color-accent)] px-2 py-0.5 type-micro font-medium text-[var(--color-surface-0)] hover:opacity-90"
                      onclick={insertLast}
                      disabled={i !== messages.length - 1 || !onInsertCommand}
                      >insert into terminal</button
                    >
                    <button
                      class="rounded-md border hairline-strong px-2 py-0.5 type-micro hover:bg-[var(--color-surface-3)]"
              ...
              {/if}
              {#if m.error}
                <div class="mt-1 type-micro text-[var(--color-danger)]">{m.error}</div>
              {/if}
            </div>
          {/if}
        {/each}
  ...
  <div class="border-t hairline px-3 py-2">
    <div class="flex items-end gap-2">
      <textarea
        class="max-h-32 min-h-[36px] flex-1 resize-none rounded-md border hairline bg-[var(--color-surface-3)] px-2 py-1.5 type-caption outline-none"
        rows="1"
        bind:value={prompt}
      ...
      {#if busy && activeStreamID}
        <button
          class="flex items-center gap-1 rounded-md border hairline-strong px-2.5 py-1.5 type-caption hover:bg-[var(--color-surface-3)]"
          onclick={stop}
        >
          <Square size="11" /> stop
        </button>
      {:else}
        <button
          class="flex items-center gap-1 rounded-md bg-[var(--color-accent)] px-3 py-1.5 type-caption font-medium text-[var(--color-surface-0)] hover:opacity-90 disabled:opacity-50"
