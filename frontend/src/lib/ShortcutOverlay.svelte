  ...
  {#snippet children()}
    ...
    <div class="flex items-center justify-between border-b hairline px-5 py-3">
      <div class="flex items-center gap-2">
        <span class="font-mono type-eyebrow text-[var(--color-text-3)]">Keyboard Shortcuts</span>
        <span class="border hairline px-1.5 py-px font-mono type-nano text-[var(--color-text-4)]">?</span>
      </div>
      <button
    ...
    <div class="divide-y divide-[var(--color-line)]">
      {#each GROUPS as group}
        <div class="px-5 py-4">
          <div class="mb-3 font-mono type-eyebrow text-[var(--color-text-4)]">
            {group.label}
          </div>
          <div class="space-y-2">
            {#each group.shortcuts as s}
              <div class="flex items-center justify-between gap-4">
                <span class="type-caption text-[var(--color-text-2)]">{s.desc}</span>
                <div class="flex shrink-0 items-center gap-1">
                  {#each s.keys as key, i}
                    {#if i > 0}
                      <span class="type-nano text-[var(--color-text-4)]">+</span>
                    {/if}
                    <kbd class="border hairline bg-[var(--color-surface-3)] px-1.5 py-0.5 font-mono type-micro text-[var(--color-text-2)]">{key}</kbd>
                  {/each}
                </div>
              </div>
            {/each}
    ...
    <div class="border-t hairline px-5 py-2.5">
      <p class="font-mono type-nano text-[var(--color-text-4)]">Press <kbd class="border hairline bg-[var(--color-surface-3)] px-1 py-px font-mono type-nano">Esc</kbd> or <kbd class="border hairline bg-[var(--color-surface-3)] px-1 py-px font-mono type-nano">?</kbd> to close</p>
    </div>
  {/snippet}
</Dialog>
